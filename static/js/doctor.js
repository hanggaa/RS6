$(document).ready(function () {
    ...
    function addDoctor(data) {
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
               $.notify("Dokter telah berhasil ditambahkan", {"status":"success"});
            table.destroy();
    ...
    function deleteDoctor(id) {
        ...
        swal({
            ...
            $.ajax(settings).done(function (response) {
                swal("Terhapus", "Dokter dihapus", "success");
                table.destroy();
                $('#datatable4 tbody').empty(); // empty in case the columns change
                getDoctor()
            });
    ...
    function updateDoctor(data, id) {
        ...
        $.ajax(settings).done(function (response) {
            $.notify("Data dokter diubah", {"status":"success"});
            $('.modal.in').modal('hide')
            table.destroy();
            $('#datatable4 tbody').empty(); // empty in case the columns change
            getDoctor()
        ...
        $.ajax(settings).done(function (response) {
            table = $('#datatable4').DataTable({
                ...
                aoColumns: [
                    ...
                    {
                        mRender: function (o) {
                            return '<button class="btn-xs btn btn-danger delete-btn" type="button">Hapus</button>';
                        }
                    }
        ]
            });
